...
@router.message(StateFilter(ClusterCreateForm), F.text == "❌ Отмена")
async def cancel_cluster_create(message: Message, state: FSMContext):
    await state.clear()
    await message.answer("❌ Создание кластера отменено.", reply_markup=get_admin_menu_keyboard())
...
@router.message(ClusterCreateForm.waiting_for_endpoint, F.text == "◀️ Назад")
async def process_cluster_endpoint_back(message: Message, state: FSMContext):
    await message.answer(
        "➕ <b>Создание кластера</b>\n\n"
        "Шаг 1/3: Введите название кластера:",
        reply_markup=get_fsm_keyboard(back=False)
...
@router.message(ClusterCreateForm.waiting_for_api_key, F.text == "◀️ Назад")
async def process_cluster_api_key_back(message: Message, state: FSMContext):
    await message.answer(
        "➕ <b>Создание кластера</b>\n\n"
        "Шаг 2/3: Введите endpoint кластера:",
        reply_markup=get_fsm_keyboard(back=True)
