...
async def process_expiration_date(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        try:
            expires_at = datetime.strptime(message.text.strip(), "%d.%m.%Y %H:%M")
        except ValueError:
...
async def process_is_admin(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        logger.info(
            f"Admin {callback.from_user.id} registered client {client.id} "
            f"for user {user_id}, expires={expires_at}, is_admin={is_admin}"
        )
        await state.clear()
        await callback.answer()
