CLIENT_INFO = """📱 <b>Клиенты для подключения:</b>

🖥 Windows / Linux → <b>Happ</b>
🤖 Android → <b>v2rayNG, Nekobox</b>
🍎 iOS → <b>Streisand, Nekobox</b>
"""


WELCOME_MESSAGE = """╔═══════════════════════╗
              🎄 ❄️  ExVPN  ❄️  🎄
╚═══════════════════════╝

👋 <b>Добро пожаловать!</b>

Перед использованием бота, пожалуйста, ознакомьтесь с нашими документами:

📋 <b>Политика конфиденциальности</b>
Описывает, как мы работаем с вашими данными

📄 <b>Пользовательское соглашение</b>
Условия использования сервиса ExVPN

Нажимая кнопку "✅ Согласен", вы принимаете условия обоих документов."""


TRIAL_MESSAGE = """🎁 <b>Вам активирован пробный период на {trial_period_days} дня!</b>

Это время, чтобы убедиться в качестве нашего сервиса.
По истечении — выберите удобный тариф в разделе 💎 Подписка."""


MAIN_MENU_MESSAGE = """╔═══════════════════════╗
              🎄 ❄️  ExVPN  ❄️  🎄
╚═══════════════════════╝

🔐 <b>Главное меню</b>

Выберите действие:"""


SUBSCRIPTION_REQUIRED = """<b>💎 Выбор тарифа</b>

Выберите подходящий тариф и получите неограниченный доступ к возможностям ExVPN

"""


SELECT_LOCATION = """🌍 <b>Выбор локации</b>

Выберите регион для подключения:"""


SELECT_KEY_MODE = """🔐 <b>Тип подключения</b>

Выберите тип подключения для региона {location}:"""


WHITELISTS_NOT_AVAILABLE = "⚠️ Белые списки пока не добавлены."


PROFILE_MESSAGE_TEMPLATE = """👤 <b>Мой профиль</b>

📱 Telegram ID: <code>{telegram_id}</code>
👤 Имя пользователя: {username}

📅 <b>Подписка:</b>
{subscription_status}

🔑 <b>Активные ключи:</b> {peers_count}"""


SUBSCRIPTION_ACTIVE_TEMPLATE = "✅ Активна до {expires_at}"
SUBSCRIPTION_EXPIRED = "❌ Подписка истекла"

REFERRAL_TEMPLATE = """🎁 <b>Реферальная программа</b>

Приглашай друзей и получай <b>{referral_bonus_days} дней</b> за каждого оплатившего пользователя.

Твоя ссылка:
<code>{ref_link}</code>

📊 <b>Статистика:</b>
• Приглашено: <b>{invited_count}</b>
• Оплатили: <b>{paid_count}</b>
• Бонус: <b>{bonus_days} дней</b>"""


KEY_RECEIVED_TEMPLATE = """✅ <b>Ключ получен!</b>

🌍 Регион: {location}
🔑 Ключ: <code>{key}</code>

Ваш конфигурационный файл готов к использованию."""


KEY_ALREADY_EXISTS_TEMPLATE = """ℹ️ <b>Ключ уже существует</b>

🌍 Регион: {location}
🔑 Ключ: <code>{key}</code>
Вы уже получали ключ для этого региона. Отправил существующий конфиг."""
