...
Если у пользователя остался старый диалог со старыми inline-кнопками, бот покажет предупреждение и отправит актуальное меню.

Также можно использовать deep link:

```text
https://t.me/ExVPN1bot?start=true
```

При получении `/start true` бот принудительно отправляет актуальное главное меню для уже зарегистрированного пользователя.
...
- изменение срока подписки клиента по `Telegram ID`;
- управление тарифами;
- просмотр статистики;
